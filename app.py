...
def dashboard():
    ...
    cur.execute("SELECT * FROM shoes,shoes_sale WHERE shoes.id_shoes=shoes_sale.id_shoes")
    rows = cur.fetchall()
...
def filter_shoes():
    if request.method == 'POST':
        ...
        # else :
            ...
            cur=mysql.connection.cursor()
            cur.execute("SELECT * FROM shoes,shoes_sale WHERE shoes.id_shoes=shoes_sale.id_shoes AND shoes.shoes_cost*(1-shoes_sale.shoes_sale_percent/100) <= %s AND shoes.shoes_cost*(1-shoes_sale.shoes_sale_percent/100) >= %s",[max_price,min_price])
            rows=cur.fetchall()
            return render_template('index.html',shoes=rows)
...
def search():
    ...
    if request.method == 'POST':
        ...
        string = "%"+search+"%"
        cur=mysql.connection.cursor()
        cur.execute("SELECT * FROM shoes,shoes_sale WHERE shoes.id_shoes=shoes_sale.id_shoes AND shoes.shoes_name LIKE %s",[string])
        rows=cur.fetchall()
        return render_template('index.html',shoes=rows,search_title=search)
    # return redirect(url_for('dashboard'))
...
@app.route("/personl/changepass" ,methods=['GET','POST'])
def change_ps():
    error=None
    if 'id' in session:
        if request.method == 'POST':
            ...
            cf_newpass=request.form['cf_new_pass']
            if newpass!=cf_newpass :
                return render_template('personal_infor.html',error="Không trùng khớp")
            cur =mysql.connection.cursor()
            cur.execute("SELECT * FROM user WHERE id_user=%s",([session['id']]))
            rows = cur.fetchall()
            if curpass !=rows[0]['password'] :
                return render_template('personal_infor.html',error="Mật khẩu không chính xác")
            else :
                cur.execute("UPDATE user SET password =%s WHERE id_user =%s",[newpass,session['id']])
                mysql.connection.commit()
                return render_template('personal_infor.html',error=error)
